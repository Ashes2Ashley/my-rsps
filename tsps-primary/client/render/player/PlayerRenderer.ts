...
export class PlayerRenderer {
    ...
    private updatePlayerGpuPass(
        ...
        const bufferCapacity = (buffer: VertexBuffer): number => {
            const legacyByteLength = (buffer as VertexBuffer & { byteLength?: number }).byteLength;
            if (typeof legacyByteLength === "number") return legacyByteLength;
            return (buffer.numItems ?? 0) * (buffer.itemSize ?? 1) * Float32Array.BYTES_PER_ELEMENT;
        };
        if (
            pass &&
            bufferCapacity(pass.vb) >= vertices.byteLength &&
            bufferCapacity(pass.ib) >= indices.byteLength
        ) {

...
uniform vec4 u_hdLightPositions[32];
uniform vec4 u_hdLightColors[32];
...
vec3 hdShade(vec3 surface, vec3 position) {
    ...
    vec3 color = base * (u_hdAmbient + u_hdDirectional * diffuse * shadow * 1.85 + pointLight) + pointLight * additiveFloor;
    color *= 0.44 + shadow * 0.56;
